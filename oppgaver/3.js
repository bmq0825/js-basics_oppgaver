...
const updatePeopleInfo = (people, cities) => {
  let combinedAge = 0;
  ...
  return { combinedAge, averageAge: combinedAge / (people.length - 1) }; // -1 for Otto;
};

const { combinedAge, averageAge } = updatePeopleInfo(people, cities);
...
const getRandomNumbers = (rollCount, dieSides = 6) =>
  new Array(rollCount)
    .fill(0)
    .map(() => Math.floor(Math.random() * dieSides) + 1);

console.log(getRandomNumbers(4));
console.log(getRandomNumbers(6));
console.log(getRandomNumbers(5, 20));
...
const cleanText = words => {
  ...
  for (const text of words) {
    normalizedWords.push(text.trim().toLowerCase());
  }
...
const doubleSwap = (string, charA, charB) =>
  string
    .split('')
    .map(char => (char === charA ? charB : char === charB ? charA : char))
    .join('');
  ...
  const words = str
    ...
    .map(word =>
      ...
      word.includes('!')
        ? word.slice(0, -1)
        : word
    ); // Better to use regex here, but keeping it simple for now

  for (const word of words) {
    if (helloMap[word]) return `HELLO oppdaget på ${helloMap[word]}.`;
  }
